...
const INTERMediator = {
  ...
  scrollBack: (bias = 0, scrollTarget = true) => {
    ...
    if (targetId && contextName) {
      ...
      if (binding) {
        ...
        if (scrollTarget !== false) {
          scrollTarget = (scrollTarget === true) ? window : scrollTarget
  ...
  localizing: () => {
    ...
    for (const node of targetNodes) {
      ...
      if (!bindValue || !INTERMediatorLib.isBindingNodeText(node)) {
...
const INTERMediatorLog = require('../../src/js/INTER-Mediator-Log')
